...
import io.github.bootystar.mybatisplus.base.injection.dto.ConditionDTO;
 ...
 * @author bootystar
 */
public interface InjectionService<T, V> extends IService<T> {
    ...
    default Class<T> entityClass() {
        return (Class<T>) Objects.requireNonNull(GenericTypeResolver.resolveTypeArguments(getClass(), InjectionService.class))[0];
    }

    @SuppressWarnings("unchecked")
    default Class<V> voClass() {
        return (Class<V>) Objects.requireNonNull(GenericTypeResolver.resolveTypeArguments(getClass(), InjectionService.class))[1];
    }
    ...
    default V voById(Serializable id) {
        return oneByDTO(new InjectionDTO().requiredConditions(ConditionDTO.builder().field(ReflectUtil.idField(entityClass())).value(id).build()));
    }
